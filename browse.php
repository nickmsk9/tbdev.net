...
if (!$all)
    if (!$_GET && $CURUSER["notifs"]) {
       ...
       $all = true;
foreach ($cats as $cat) {
    // Исправлено: $car[id] на $cat['id'] и побитовое И на логическое И
    $all = $all && isset($_GET["c{$cat['id']}"]);
    
    if (isset($_GET["c{$cat['id']}"])) {
        $wherecatina[] = $cat['id']; // Исправлено: $cat[id] на $cat['id']
        $addparam .= "c{$cat['id']}=1&amp;";
    }
}
        }
...
if ($count) {
    if ($addparam != "") {
        if ($pagerlink != "") {
            if ($addparam[strlen($addparam) - 1] != ";") { // Исправлено: {} на []
    ...
    $res = sql_query($query) or die(mysql_error()); // Внимание: здесь тоже устаревший mysql_error()
} else
    unset($res);
if (isset($cleansearchstr))
